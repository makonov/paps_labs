...
Для данной лабораторной работы выбрана **архитектурная ката** ненастоящего проекта (представлена в файле [kata.md](./kata.md)), так как дипломная работа не может быть использована в рамках этой лабораторной. 

В данной работе реализуется **сервис управления конференциями** (Conference Management Service) — основной backend-сервис системы, который предоставляет REST API для работы с конференциями, докладами, спикерами, участниками, расписанием, голосованиями и материалами (слайдами).
